...
test('montarPecas exige as variáveis obrigatórias', () => {
  ...
  assert.throws(() => montarPecas({ PLANILHA_ID: 'x', GOOGLE_SERVICE_ACCOUNT_JSON: '{}' }), /SESSION_SECRET/);
});

test('confiarProxy só liga em produção', () => {
  const dir = mkdtempSync(join(tmpdir(), 'frota-p-'));
  const base = {
    PLANILHA_ID: 'x', GOOGLE_SERVICE_ACCOUNT_JSON: '{}', SESSION_SECRET: 'seg',
